...
//
//
// - fakeCollections:{}   Map from collection name to the data we need
//     (fields we want to include in projected data)
var Livedb = exports.client = function(options) {
  ...
  this.fakeCollections = options.fakeCollections || {};
...
Livedb.prototype.submit = function(cName, docName, opData, options, callback) {
  ...
  if(this.fakeCollections[cName]) return callback("can't submit to fake collection");
...
Livedb.prototype.queryFetch = function(cName, query, opts, callback) {
  ...
  var queryOptions = {};
  getQueryFields(self, cName, queryOptions);

  db.query(this, cName, query, queryOptions, function(err, resultSet) {
    if (self.sdc) {
...
Livedb.prototype.query = function(index, query, opts, callback) {
  ...
  this.subscribeChannels(channels, function(err, stream) {
    ...
    var queryOptions = {};
    getQueryFields(self, index, queryOptions);

    db.query(self, index, query, queryOptions, function(err, resultSet) {
      ...
      if (poll) {
        var runQuery = rateLimit(delay, function() {
          ...
          var dbStart = Date.now();
          db.query(self, index, query, queryOptions, function(err, newResultset) {
...
// convenience function for dealing with query fields when using a fake collection to do projections
function getQueryFields(db, index, queryOptions) {
  if(db.fakeCollections[index] && db.fakeCollections[index].length) {
    queryOptions.fields = db.fakeCollections[index];
  }
}
