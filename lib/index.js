...
Livedb.prototype.query = function(index, query, opts, callback) {
  ...
  this.subscribeChannels(channels, function(err, stream) {
    ...
    var queryOptions = {};
    console.log("FAKE COLLECTIONS", self.fakeCollections);
    if(self.fakeCollections[index] && self.fakeCollections[index].length) {
      queryOptions.fields = self.fakeCollections[index];
    }
