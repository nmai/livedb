...
exports.filterOp = function(op, fields, type) {
  // we modify the op in place.
  if(type.filterOp) {
    var newOp = type.filterOp(op, fields);
    for (var i = 0, l = op.length; i < l; i++) { op.pop() } // better cache op.length or this wont work.
    for (var i = 0; i < newOp.length; i++) { op.push(newOp[i]) }
  } else {
    return "Not implemented";
  }
};

exports.filterSnapshot = function(snapshot, fields, type) {
  // we modify the snapshot in place.
  if(type.filterSnapshot) {
    snapshot.data = type.filterSnapshot(snapshot.data, fields);
  } else {
    return "Not implemented";
  }
};
